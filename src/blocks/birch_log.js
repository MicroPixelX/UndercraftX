...
const side = generateTexture(16, 16, (ctx, w, h) => {
  ...
  for(let i=0;i<6;i++){const y=Math.floor(Math.random()*h),sx=Math.floor(Math.random()*(w-5));ctx.fillStyle='#4a3828';ctx.fillRect(sx,y,3+Math.floor(Math.random()*5),1);}
  for(let x=0;x<w;x++)if(Math.random()>0.7){ctx.fillStyle='rgba(180,170,150,0.5)';ctx.fillRect(x,0,1,h);}
});
registerBlock(11, { name: 'Bétula', solid: true, createMaterials: () => createBlockMaterial(top, side, top) });
